...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        denominator = 1.0 + np.exp(-y * (self.bias + X @ self.weight))
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        exp_term = -y * (self.bias + np.dot(X, self.weight))
        return np.mean(np.log(1.0 + np.exp(exp_term))) + self._lambda * np.dot(self.weight.T, self.weight)
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        y_mu_term = y * (self.mu(X, y) - 1.0)
        return np.mean(y_mu_term[:, None] * X, axis = 0) + 2 * self._lambda * self.weight
    ...
    def train(
        ...
        for i in range(epochs):
            for j in range(num_batches):
                train_i = RNG.choice(len(X_train), batch_size, replace = False)
                self.step(X_train[train_i], y_train[train_i], learning_rate)
